...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const [backendUser, setBackendUser] = useState<VKAuthResponse["user"] | null>(null)
      ...
      try {
        ...
        if (isVK) {
          ...
          try {
            ...
            console.error('❌ Пользователь отказал в доступе или произошла ошибка:', tokenError)
          ...
          try {
            ...
            const authResponse = await authService.vkAuth(vkParams, vkAccessToken);
  ...
  return (
    <AuthContext.Provider value={{ user, backendUser, accessToken, tokenScope, isLoading, isVKEnvironment, isNewUser }}>
      {children}
